...
export default {
  ...
  methods: {
    formatearFechaParaInput(fecha) {
      ...
      return `${mes.padStart(2, '0')}-${anio}-${dia.padStart(2, '0')}`;
    },
    async cargarTareas() {
      try {
        const res = await axios.get('http://localhost:3000/users/cliente/tareas');
        this.tareas = res.data.map(tarea => ({
          ...tarea,
          fecha: this.formatearFechaParaInput(tarea.fecha)
        }));
      } catch (error) {
    ...
    async crearTarea() {
      try {
        await axios.post('http://localhost:3000/users/cliente/tareas', this.nuevaTarea);
    ...
    async actualizarTarea() {
      try {
        await axios.put(`http://localhost:3000/users/cliente/tareas/${this.idEditando}`, this.nuevaTarea);
        this.resetFormulario();
        this.modo = 'lista';
        await this.cargarTareas();
      } catch (error) {
    ...
    async eliminarTarea(id) {
      if (confirm('¿Deseas eliminar esta tarea?')) {
        try {
          await axios.delete(`http://localhost:3000/users/cliente/tareas/${id}`);
          await this.cargarTareas();
        } catch (error) {
